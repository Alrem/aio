...
sudo ./create-config-drive.sh -k ~/.ssh/id_rsa.pub -h cfg01.virtual-mcp11-aio.local /var/lib/libvirt/aio_mcp.iso
...
sudo virt-install --name cfg01.virtual-mcp11-aio.local --ram 16384 --vcpus=4 --cpu host \
...
sudo virsh net-update mcp_ext add \
       	ip-dhcp-host "<host mac='$(virsh domiflist cfg01.virtual-mcp11-aio.local | grep mcp_ext | awk '{print $5}')' name='cfg01.virtual-mcp11-aio.local' ip='10.18.0.2' /> "\
       	--live --config

sudo virsh start cfg01.virtual-mcp11-aio.local
