...
./create-config-drive.sh -k ~/.ssh/id_rsa.pub -h mcp /var/lib/libvirt/aio_mcp.iso 
cp xenial-server-cloudimg-amd64-disk1.img /var/lib/libvirt/aio_mcp.img
qemu-img resize /var/lib/libvirt/aio_mcp.img 120G
...
virsh net-define mcp_ext_net.xml
virsh net-autostart mcp_ext
virsh net-start mcp_ext

virt-install --name aio_mcp --ram 16384 --vcpus=4 --cpu host \
